...
type requestHeaderAuthRequestHandler struct {
	...
	// uidHeaders are the headers to check (in order, case-insensitively) for uid. The first header with a value wins.
...
func NewSecure(clientCA string, proxyClientNames, nameHeaders, uidHeaders, groupHeaders, extraHeaderPrefixes []string) (authenticator.Request, error) {
	if len(clientCA) == 0 {
...
func (a *requestHeaderAuthRequestHandler) AuthenticateRequest(req *http.Request) (*authenticator.Response, bool, error) {
	...
	// TODO (BEN): Once the user is authenticated, the headers are deleted so that code in layers down cannot
	//   make decisions based on these values.  This is best practices in security
	// clear headers used for authentication
	for _, headerName := range a.nameHeaders.Value() {
		req.Header.Del(headerName)
	}
	for _, headerName := range a.uidHeaders.Value() {
		req.Header.Del(headerName)
	}
